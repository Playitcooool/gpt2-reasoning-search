...
RL uses Brave as its knowledge source when `BRAVE_SEARCH_API_KEY` is exported, the Qwen3.5-2B judge
by default, and at most three searches per rollout. Brave results use a persistent cache and fall
back to local BM25 only when the API is unavailable. Set `USE_LLM_JUDGE=0` for the deterministic
ablation. Detailed JSONL schemas and advanced CLI equivalents are in [the search-RL guide](docs/SEARCH_RL.md).
...
and regularizes against a frozen copy of the tool-SFT checkpoint. When a Brave key is exported, live
Brave is the knowledge source and local BM25 is used only for API outage/quota fallback. Lookup-
required answers receive correctness and judge reward only after they cite matching returned evidence.
Its tokens do not alter the audited 70/30 pretraining ratio. See [search RL](docs/SEARCH_RL.md).
